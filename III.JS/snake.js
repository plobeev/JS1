...
var mine_timer; // Таймер препятствий
...
function startGame() {
    ...
    mine_timer = setInterval(createMine, MINE_SPEED);//каждые 200мс запускаем функцию move
}
...
function haveFood(unit) {
    ...
    if (unit.classList.contains('food-unit')) {
        ...
        createFood();
        score++;
        document.getElementById("score_block").innerText="Счет :" + score;
    }
    if (unit.classList.contains('mine-unit')) {
        finishTheGame();
    }
    return check;
}
...
function createMine() {
    ...
    while (!mineCreated) { 
        ...
        if (!mine_cell.classList.contains('snake-unit')) {
            mine_cell.classList.toggle("mine_alert-unit");            
            mineCreated = true;            
        }
        setTimeout(changeToMine, 1500);
    }
}

function changeToMine(){
    document.querySelector(".mine_alert-unit").classList.add("mine-unit");
    document.querySelector(".mine_alert-unit").classList.remove("mine_alert-unit")
}
/**
